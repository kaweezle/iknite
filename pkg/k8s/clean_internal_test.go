...
func TestProcessMounts_EvalSymlinksNotExist(t *testing.T) {
	...
	err := processMounts(mockH, testMountPath, false, "test", false, logger)
	req.NoError(err)
}
...
func TestProcessMounts_DryRun_WithMatch(t *testing.T) {
	...
	logger := testutil.TestLogger(t)
	err := processMounts(mockH, testMountPath, false, "Unmounting", true /* isDryRun */, logger)
	req.NoError(err)
}
...
func TestProcessMounts_Unmount_Success(t *testing.T) {
	...
	logger := testutil.TestLogger(t)
	err := processMounts(mockH, testMountPath, false /* remove */, "Unmounting", false, logger)
	req.NoError(err)
}
...
func TestProcessMounts_Unmount_Error_ContinuesAndReturnsNil(t *testing.T) {
	...
	err := processMounts(mockH, testMountPath, false, "Unmounting", false, logger)
	req.NoError(err)
}
...
func TestProcessMounts_Remove_Success(t *testing.T) {
	...
	logger := testutil.TestLogger(t)
	err := processMounts(mockH, testMountPath, true /* remove */, "Unmounting and removing", false, logger)
	req.NoError(err)
}
...
func TestProcessMounts_Remove_Error_ContinuesAndReturnsNil(t *testing.T) {
	...
	logger := testutil.TestLogger(t)
	err := processMounts(mockH, testMountPath, true, "Unmounting and removing", false, logger)
	req.NoError(err)
}
