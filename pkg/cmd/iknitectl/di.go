...
func NewContainer(opts *RootOptions) (*dig.Container, error) {
	...
	if err := c.Provide(context.Background); err != nil {
		return nil, fmt.Errorf("failed to provide context: %w", err)
	}

	if err := c.Provide(func() types.CmdOut { return os.Stdout }); err != nil {
		return nil, fmt.Errorf("failed to provide CmdOut: %w", err)
	}
...
var hostInterfacesOption = dig.As(
...
func provideOptionsAndHostFromOpts(c *dig.Container, opts *RootOptions) error {
	if opts == nil {
		if err := c.Provide(func() *RootOptions { return NewRootOptions(nil) }); err != nil {
	...
	if err := c.Provide(func(opts *RootOptions) host.Host { return opts.host }); err != nil {
		return fmt.Errorf("failed to provide host: %w", err)
	}
	if err := c.Provide(func(opts *RootOptions) host.Host { return opts.host }, hostInterfacesOption); err != nil {
		return fmt.Errorf("failed to provide host with interfaces: %w", err)
	}
...
func ProvideCommand(c Provider, cmd *cobra.Command, args []string) error {
	...
	if err := c.Decorate(
		func(_ types.CmdOut, command *cobra.Command) types.CmdOut {
