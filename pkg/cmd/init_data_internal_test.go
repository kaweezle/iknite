...
func decodeStatusResponse(t *testing.T, srv *ikniteServer.IkniteServer) *v1alpha1.IkniteCluster {
	...
	require.Equalf(t, http.StatusOK, rec.Code,
		"expected status code %d, got %d: %s", http.StatusOK, rec.Code, rec.Body.String())
...
func TestInitDataStatusServerAndMDNS(t *testing.T) {
	...
	ch, unsubscribe := data.RegisterIkniteClusterListener()
	go func() {
		for update := range ch {
			statusServer.SetCluster(update)
			updateCh <- struct{}{}
		}
	}()

	data.RegisterShutdownHook("statusServer", func() error {
		unsubscribe()
	...
	cluster.Spec.ClusterName = "updated-cluster"
	...
	conn := createTestMDNSConn(t)
	closed := false
	data.RegisterShutdownHook("mdns", func() error {
		closed = true
		return conn.Close()
	})
	...
	req.NoError(err)
	// Test that mdns connection is closed and error from close is handled
	req.True(closed)
	req.True(statusServer.IsShutDown())
}
